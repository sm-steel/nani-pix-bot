...
def _create_setup_game(session_factory, *, starter_id: int = 1, file_id: str = "file123") -> None:
    with session_factory() as session:
        session.add(Player(telegram_user_id=starter_id))
        session.commit()
        game_service.create_setup_game(session, starter_id=starter_id, original_file_id=file_id)
        session.commit()
...
def _make_callback_context(session_factory, **extra_bot_data) -> MagicMock:
    context = _make_context(
        session_factory, game_topic_id=7, anilist_client=MagicMock(), **extra_bot_data
    )
...
async def test_pick_callback_handler_survives_a_restart_between_search_and_pick(
    ...
    get_by_id_mock.assert_awaited_once_with(context.bot_data["anilist_client"], 99)
    context.bot.send_photo.assert_awaited_once()
