...
from nani_pix_bot.commands import setautostart as setautostart_module
...
async def test_setautostart_rejects_non_admin(session_factory) -> None:
    update = _make_update(user_id=2)
    context = _make_context(session_factory, args=["on"])

    await setautostart_module.setautostart_command(
...
async def test_setautostart_ignores_group_chat(session_factory) -> None:
...
async def test_setautostart_rejects_invalid_argument(session_factory) -> None:
...
async def test_setautostart_on_enables_autostart(session_factory) -> None:
    update = _make_update(user_id=1)
    context = _make_context(session_factory, admin_ids={1}, args=["on"])
...
async def test_setautostart_off_disables_autostart(session_factory) -> None:
    with session_factory() as session:
